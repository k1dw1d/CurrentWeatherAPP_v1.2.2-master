...
public class Function {
    ...
    private static final String OPEN_WEATHER_MAP_COORD_URL =
            "http://api.openweathermap.org/data/2.5/weather?lat=%s&lon=%s&units=metric";


    private static final String OPEN_WEATHER_MAP_API = "28352cd6a33346c0b8aa305813460433";

    //Creating a string with the needed figures of the weather from the font packet
        ...
        protected void onPostExecute(JSONObject json) {
            try {
                if(json != null){
                    ...
                    //Creating JSONObjects for calling the elements from openweather API MAP
                    ...
                    //Accessing the needed data about weather from json objects
                    String city = json.getString("name").toUpperCase(Locale.US) + ", " + json.getJSONObject("sys").getString("country");
                    String description = detailsObj.getString("description").toUpperCase(Locale.US);
                    String temperature = mainObj.getInt("temp")+ "°C";
                    ...
                    //Passes the created above strings to the processFinish from the AsyncResponse interface
                    //They will be inserted in the order they are written
    ...
    //Creating the function that access the weather map and uses the lat and lon we will later input
    public static JSONObject getWeatherJSON(String lat, String lon){
        try {
            URL url = new URL(String.format(OPEN_WEATHER_MAP_COORD_URL, lat, lon));
